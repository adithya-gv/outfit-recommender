...
We used two neural networks to try to cluster the data. We started with a self designed shallow network with 4 dense layers. However its accuracy was pretty bad, so we decided to try something else. 
...
We then switched to a trained FastAI deep neural network. Here is the structure and training data for this network. 

![Figure 14](docs/assets/fastapiNNdetail.png)
